...
def opt_model(workspace_folder):
    ...
    """
    Define expressions for Energy Hub (EH) capacity
    """
    ...
    def global_cost_rule(model):
        ...
        return wf_total_cost + eh_total_cost + ec1_total_cost + ec2_total_cost + onss_total_cost + onc_total_cost + onss_total_cap_aux
    ...
    # Define new constraint for onshore substation connections
    def onss_cap_connect_rule(model, onss):
        """
        Ensure the capacity of each onshore substation is at least the total incoming capacity from connected energy hubs,
        adjust for the distributed capacity to other substations, and account for capacity received from other substations.
        """
        ...
        return model.onss_cap_var[onss] >= connect_from_eh + receive_from_others - distribute_to_others
    ...
    def save_results(model, workspace_folder):
        ...
        for onss1, onss2 in model.viable_onc_ids:
            if model.onc_cap_var[onss1, onss2].value is not None and model.onc_cap_var[onss1, onss2].value > 0:
                ...
                dist3 = par_f(haversine(model.onss_lon[onss1], model.onss_lat[onss1], model.onss_lon[onss2], model.onss_lat[onss2]))
                onc_data.append((onc_id_counter, onss1, onss2, model.onss_lon[onss1], model.onss_lat[onss1], model.onss_lon[onss2], model.onss_lat[onss2], dist3, onc_cap, onc_cost))
                onc_id_counter += 1
