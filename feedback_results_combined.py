...
def add_fields_if_not_exist(feature_class, fields):
    """
    Adds fields to a feature class if they do not already exist.
    ...
    if fields_to_add:
        ...
        arcpy.AddMessage(f"Added fields to {feature_class}: {fields_to_add}")
    else:
...
def create_polyline_feature_layer(map_obj, existing_layer_names, npy_file_path, workspace_folder, layer_name, results_folder):
    """
    Creates a polyline feature layer from .npy data and adds it to the current project map.

    Parameters:
    - map_obj: The current map object.
    - existing_layer_names: List of existing layer names in the current map.
    ...
    add_fields_if_not_exist(lines_path, fields)
...
def create_point_feature_layer(map_obj, existing_layer_names, npy_file_path, workspace_folder, layer_name, results_folder, include_rate=False):
    """
    ...
    # Add fields if they do not already exist
    add_fields_if_not_exist(points_path, fields)
...
def create_polygon_feature_layer_from_points(map_obj, existing_layer_names, wfa_layer, wf_layer, workspace_folder, layer_name, results_folder):
    """
...
def process_feature_layers(npy_file_paths, feature_layer_dir, results_folder):
    ...
    for file_path in npy_file_paths:
        ...
        if 'wf' in file_name:
            create_point_feature_layer(map_obj, existing_layer_names, file_path, feature_layer_dir, file_name, results_folder, include_rate=True)
            added_wf_layers.append(layer_name)
        elif any(identifier in file_name for identifier in ['eh', 'onss']):
            create_point_feature_layer(map_obj, existing_layer_names, file_path, feature_layer_dir, file_name, results_folder)
        elif any(identifier in file_name for identifier in ['ec1', 'ec2', 'ec3', 'onc']):
            create_polyline_feature_layer(map_obj, existing_layer_names, file_path, feature_layer_dir, file_name, results_folder)
    ...
    for wf_layer_name in added_wf_layers:
        ...
        if wf_layer:
            ...
            create_polygon_feature_layer_from_points(map_obj, existing_layer_names, wfa_layer, wf_layer, feature_layer_dir, new_layer_name, results_folder)
...
if __name__ == "__main__":
    ...
    results_folder = 'SF-D-I-LIM100-IC40'
