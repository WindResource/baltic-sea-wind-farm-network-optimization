...
def create_wind_turbine_shapefile(input_folder: str, turbine_spacing: float, output_folder: str, map_frame_name: str, utm_zone: int) -> None:
    """
            ...
            while y < bounding_box.YMax:
                while x < bounding_box.XMax:
                    if shape.contains(arcpy.Point(x, y)):
                        turbine_id, capacity = f"Turbine_{turbine_count}", 0.0
                        cursor.insertRow((arcpy.Point(x, y), turbine_id, capacity, x, y))
                        turbine_count += 1

                    x += turbine_spacing

                x, y = bounding_box.XMin, y + turbine_spacing
...
if __name__ == "__main__":
    # Get the input folder, output folder, map frame name, turbine spacing, and UTM zone from the user input
    ...
    utm_zone: int = int(arcpy.GetParameterAsText(4))
    ...
    if not os.path.isdir(output_folder):
        ...
        create_wind_turbine_shapefile(input_folder, turbine_spacing, output_folder, map_frame_name, utm_zone)
