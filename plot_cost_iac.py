...
def plot_costs_vs_distance():
    ...
    desired_order = ['Total Cost', 'Equipment Cost', 'Total Operating Cost', 'Installation Cost', 'Decommissioning Cost']
    ...
    fig.legend(ordered_lines, ordered_labels, bbox_to_anchor=(0.5, 1.04), loc='center', ncol=2, frameon=False)
...
def plot_costs_vs_capacity():
    ...
    # Plotting the larger range
    axs[0].plot(capacities, total_costs, label='Total Cost')
    axs[0].plot(capacities, equip_costs, label='Equipment Cost')
    axs[0].plot(capacities, inst_costs, label='Installation Cost')
    axs[0].plot(capacities, total_ope_costs, label='Total Operating Cost')
    axs[0].plot(capacities, deco_costs, label='Decommissioning Cost')
    ...
    axs[0].text(axs[0].get_xlim()[1] * 0.02, axs[0].get_ylim()[1] * 0.99, f'$Distance={distance}m$', ha='left', va='top', fontsize=11)
